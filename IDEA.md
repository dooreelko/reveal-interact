...
Host gets the session token and user IDs, their ID is stored in the backend (host table - token, uid) marking them as a host
- Host establishes a websocket connection under /ws/v1/session/<token>/host/<uid>/pipe
- Based on the session token, a QR code leads users to the session interaction page
- On page load an API call is made to /api/v1/login that sets the user's ID for the session (store in the cookie?)
User ID is stored in the user table (token, uid)
...
This information is stored in the session table (token, page, state)
This triggers a broadcast message to all connected users of the session
- During the session users can query the current state using /api/v1/session/<token>/state/
